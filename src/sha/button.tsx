...
const buttonVariants = cva(
  'max-w-full inline-flex items-center justify-center gap-2 font-sans whitespace-nowrap rounded-md text-base font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        filled:
          'border border-border bg-primary text-primary-foreground hover:bg-secondary hover:text-primary hover:border-secondary',
        destructive: 'bg-destructive text-destructive-foreground shadow-sm hover:bg-destructive/90',
        outline:
          'border border-border bg-transparent shadow-sm hover:bg-accent hover:text-accent-foreground hover:shadow-none',
        secondary: 'bg-secondary text-secondary-foreground shadow-sm hover:bg-secondary/80',
        ghost:
          'bg-transparent [@media(hover:hover)]:hover:bg-accent [@media(hover:hover)]:hover:text-accent-foreground [@media(hover:hover)]:hover:shadow-none [@media(hover:none)]:active:bg-accent [@media(hover:none)]:active:text-accent-foreground [@media(hover:none)]:active:shadow-none',
